...
**Live tool:** [tmx-language-conversion.vercel.app](https://tmx-language-conversion.vercel.app/)
**Repository:** [github.com/zirafinjezik/tmx-language-conversion](https://github.com/zirafinjezik/tmx-language-conversion)

Two ways to use it:

- **Browser tool** ([live](https://tmx-language-conversion.vercel.app/)) for one-off conversions. Drop a TMX in, check the locales it contains, pick a new source code, download the corrected file.
...
Open [the live tool](https://tmx-language-conversion.vercel.app/) (or `index.html` locally). Drop in a TMX file and it reads the header source language, lists every locale present, and counts the translation units. Pick a from/to code (common locales are suggested, or type any BCP-47 tag), convert, and download.
...
python scripts/convert_language_code.py
```

Output is saved to `sample_tmx/converted_en-GB_python.tmx` by default.

To convert your own file, edit the paths in the script:

```python
input_file  = "path/to/your/input.tmx"
output_file = "path/to/your/output.tmx"
```

To use a different pair, edit the `replacements` dictionary:

```python
replacements = {
    'xml:lang="en-GB"': 'xml:lang="en-AU"',
    'srclang="en-GB"':  'srclang="en-AU"',
    'adminlang="en-GB"': 'adminlang="en-AU"',
}
```
